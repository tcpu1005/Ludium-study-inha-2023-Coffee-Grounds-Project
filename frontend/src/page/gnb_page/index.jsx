...
const Gnb_page = () => {
  ...
  return (
    <>
      ...
      <Button_4 onClick={() => all_user_page_fn("/emission")}>나의 배출 목록 및 배출하기</Button_4>


      {/* 7_MY_PAGE_CAFE_COFFEE_INPUT */}
      <Button_4 onClick={() => all_user_page_fn("/emission/input")}>배출 입력하기(개발 전용)</Button_4>


      {/* 8_MY_PAGE_CAFE_INSENTIVE */}
      <Button_4 onClick={() => all_user_page_fn("/insentive")}>나의 인센티브 확인하기</Button_4>


      {/* 9_MY_PAGE_CAFE_TOKEN */}
      <Button_4 onClick={() => all_user_page_fn("/insentive/token")}>나의 토큰 확인하기(+)</Button_4>


      {/* 10_MY_PAGE_CAFE_TOKEN_EXCHANGE */}
      <Button_4 onClick={() => all_user_page_fn("/insentive/token/exchange")}>토큰 환전하기(+)</Button_4>
      ...
      <Button_4 onClick={() => all_user_page_fn("/collection/input")}>수거 입력하기(개발 전용)</Button_4>


      {/* 13_MY_PAGE_COLLECTOR_CHECK */}
      {/* 나의 수거 완료 목록 조회 + 수정하기 버튼 (준비중) + 검증 완료 버튼 (해당 페이지 수거자만 접근 가능) */}
      <Button_4 onClick={() => all_user_page_fn("/collection/check")}>나의 수거 목록 확인하기(+)</Button_4>
